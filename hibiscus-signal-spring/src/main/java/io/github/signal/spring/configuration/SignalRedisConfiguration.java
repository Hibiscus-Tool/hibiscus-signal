...
import io.github.signal.core.persistent.RedisSignalPersistence;
...
public class SignalRedisConfiguration {
    ...
    public SignalRedisConfiguration(SignalProperties signalProperties) {
        ...
        log.info("Redis Config Is Ready, Connection Address: {}:{}",
    ...
    public RedisConnectionFactory redisConnectionFactory() {
        ...
        log.info("Redis Connection Config: {}:{}, database: {}",
    ...
    /**
     * 手动注册 RedisSignalPersistence 作为 Spring Bean
     */
    @Bean
    @ConditionalOnClass({RedisTemplate.class, LettuceConnectionFactory.class})
    @ConditionalOnProperty(name = "hibiscus.redis-enabled", havingValue = "true", matchIfMissing = false)
    public RedisSignalPersistence redisSignalPersistence() {
        return new RedisSignalPersistence(redisTemplate, signalProperties, objectMapper);
    }
}
