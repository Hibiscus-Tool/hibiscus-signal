 ...
 * @author heathcetide
 */
public class DeadLetterQueueManager<S, T> {

    private static final Logger log = LoggerFactory.getLogger(DeadLetterQueueManager.class);

    // 死信事件存储
    private final List<DeadLetterEvent<S, T>> deadLetterEvents = new CopyOnWriteArrayList<>();

    // 事件处理器映射
    private final Map<String, SignalHandler<S, T>> eventHandlers = new ConcurrentHashMap<>();
     ...
     * 添加死信事件
     */
    public void addDeadLetterEvent(DeadLetterEvent<S, T> event) {
        synchronized (deadLetterEvents) {
            // 检查容量限制
            if (deadLetterEvents.size() >= maxDeadLetterEvents) {
                // 移除最旧的事件
                DeadLetterEvent<S, T> oldestEvent = deadLetterEvents.remove(0);
     ...
     * 注册事件处理器
     */
    public void registerEventHandler(String eventName, SignalHandler<S, T> handler) {
    ...
    public boolean retryDeadLetterEvent(String eventId) {
        DeadLetterEvent<S, T> event = findEventById(eventId);
        if (event == null) {
        ...
        SignalHandler<S, T> handler = eventHandlers.get(event.getEventName());
        if (handler == null) {
     ...
     * 批量重试死信事件
     */
    public DeadLetterRetryResult batchRetryDeadLetterEvents(Predicate<DeadLetterEvent<S, T>> filter) {
        List<DeadLetterEvent<S, T>> eventsToRetry = deadLetterEvents.stream()
                .filter(filter)
        ...
        for (DeadLetterEvent<S, T> event : eventsToRetry) {
            try {
     ...
     * 查询死信事件
     */
    public List<DeadLetterEvent<S, T>> queryDeadLetterEvents(Predicate<DeadLetterEvent<S, T>> filter) {
     ...
     * 根据事件名称查询
     */
    public List<DeadLetterEvent<S, T>> queryByEventName(String eventName) {
     ...
     * 根据状态查询
     */
    public List<DeadLetterEvent<S, T>> queryByStatus(DeadLetterEvent.DeadLetterStatus status) {
     ...
     * 根据失败原因查询
     */
    public List<DeadLetterEvent<S, T>> queryByFailureReason(String failureReason) {
    ...
    public DeadLetterQueueStats getQueueStats() {
        ...
        stats.setPendingEvents((int) deadLetterEvents.stream()
                .filter(event -> event.getStatus() == DeadLetterEvent.DeadLetterStatus.PENDING)
                .count());
        stats.setRetryingEvents((int) deadLetterEvents.stream()
                .filter(event -> event.getStatus() == DeadLetterEvent.DeadLetterStatus.RETRYING)
                .count());
        stats.setRetrySuccessEvents((int) deadLetterEvents.stream()
                .filter(event -> event.getStatus() == DeadLetterEvent.DeadLetterStatus.RETRY_SUCCESS)
                .count());
        stats.setRetryFailedEvents((int) deadLetterEvents.stream()
                .filter(event -> event.getStatus() == DeadLetterEvent.DeadLetterStatus.RETRY_FAILED)
                .count());
        stats.setProcessedEvents((int) deadLetterEvents.stream()
                .filter(event -> event.getStatus() == DeadLetterEvent.DeadLetterStatus.PROCESSED)
                .count());
    ...
    public int cleanupExpiredEvents() {
        ...
        List<DeadLetterEvent<S, T>> expiredEvents = deadLetterEvents.stream()
     ...
     * 根据ID查找事件
     */
    private DeadLetterEvent<S, T> findEventById(String eventId) {
        return deadLetterEvents.stream()
