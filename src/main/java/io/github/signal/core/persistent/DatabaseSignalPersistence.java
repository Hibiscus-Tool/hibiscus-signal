...
import io.github.signal.core.model.EventRecord;
...
public class DatabaseSignalPersistence {
    ...
    public DatabaseSignalPersistence(EventRecordRepository eventRecordRepository, ObjectMapper objectMapper) {
     ...
     */
    @Transactional
    public EventRecord saveEventRecord(Sig sigHandler, SignalConfig config,
        ...
        try {
            ...
            eventRecord.setParamsData(objectMapper.writeValueAsString(sigHandler));
     ...
     * 从事件记录恢复SigHandler
     */
    public Sig restoreSigHandler(EventRecord eventRecord) {
        try {
            if (eventRecord.getHandlerInfo() != null) {
                return objectMapper.readValue(eventRecord.getHandlerInfo(), Sig.class);
            }
        } catch (JsonProcessingException e) {
