...
import io.github.signal.core.tractional.TransactionalSignalHandler;
...
public class SignalRegistry<S, T> {
    ...
    public SignalRegistry(SignalMetrics metrics, EventTransactionManager<S, T> transactionManager, DeadLetterQueueManager<S, T> deadLetterQueueManager) {
    ...
    public long registerHandler(String event, SignalHandler<S, T> handler, SignalConfig signalConfig) {
        ...
        // 如果启用了事务支持，包装为事务感知的处理器
        if (transactionManager != null && deadLetterQueueManager != null &&
                signalConfig.getTransactionConfig() != null &&
                signalConfig.getTransactionConfig().isEnableTransaction()) {

            finalHandler = new TransactionalSignalHandler<>(
                    handler, signalConfig, transactionManager, deadLetterQueueManager
            );
