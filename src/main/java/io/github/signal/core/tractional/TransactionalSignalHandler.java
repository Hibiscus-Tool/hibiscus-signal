...
public class TransactionalSignalHandler<S, T> implements SignalHandler<S, T> {
    ...
    public void handle(Envelope<S, T> envelope) {
        if (!config.getTransactionConfig().isEnableTransaction()) {
            // 如果未启用事务，直接执行
        ...
        try {
            ...
            // 如果启用了死信队列，发送到死信队列
            if (config.getTransactionConfig().isEnableDeadLetter() && deadLetterQueueManager != null) {
                handleDeadLetter(envelope, e);
            }
    ...
    private void handleDeadLetter(Envelope<S, T> envelope, Exception error) {
        try {
            DeadLetterEvent<S, T> deadLetterEvent = new DeadLetterEvent<>(
                    ...
                    envelope.getPayload(),
                    error,
                    config.getTransactionConfig().getMaxRetries()
            );
